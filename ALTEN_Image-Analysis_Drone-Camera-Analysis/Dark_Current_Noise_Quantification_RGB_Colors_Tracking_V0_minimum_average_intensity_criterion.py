...
def save_one_frame(frame_number,name_to_save_string):
    # function to exctrat one given frame from a video
    try:
        os.mkdir(output_loc)
    except OSError:
        pass
...
Intensity_Threshold = 70 # intensité moyenne maximale admise pour garder une image (lors de la pré-sélection)
...
delta_epsilon_t = 0.01 # permet de ne pas avoir de chevauchement entre al dernière image de l'intervalle précédent et la première image de l'intervalle suivant
