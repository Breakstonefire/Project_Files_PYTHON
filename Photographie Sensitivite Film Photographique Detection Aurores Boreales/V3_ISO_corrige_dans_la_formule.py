...
# 2. Le diamètre d'ouverture du diaphragme (à distance focale fixée et constante)
...
def find_zmin_zmax_in_3D_matrix(L_RSB):
    length = len(L_RSB)
    zmin,zmax = 0,0
    L_zmean = []
    for i in range(length):
        ...
        L_zmean.append(zm_i)
    zmin,zmax = min(L_zmean),max(L_zmean)
    return zmin,zmax
...
print("len(Texpo)=",len(L_Texposition))
facteur_reduction = 7
L_Texposition = reduction_nombre_elems_liste(L_Texposition,facteur_reduction)
print(L_Texposition,"len(Texpo)=",len(L_Texposition))
...
#t_fin_calcul = time.clock()
#print("Calcul terminé en ",t_fin_calcul-t_debut_calcul," secondes")
...
zmin,zmax = find_zmin_zmax_in_3D_matrix(L_RSB)
print(zmin)
print(zmax)
...
len_RSB = len(L_RSB)
for i in range(len_RSB):
    red = 1-(i/(len_RSB-1))
    green = 0
    blue = i/(len_RSB-1)
    Matrix_2D = np.array(L_RSB[i])
    # Plot the surface.
    ax.plot_surface(Lx, Ly, Matrix_2D, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ax.scatter(Lx, Ly, Matrix_2D, color=[red,green,blue], marker = 'o')
