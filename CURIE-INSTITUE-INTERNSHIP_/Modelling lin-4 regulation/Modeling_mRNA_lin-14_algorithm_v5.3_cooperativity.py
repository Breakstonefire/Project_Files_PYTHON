...
def Irate_NHR23_NHR85_lin14(Imax,Dn,ka_specific_common_value,Kn,Kr,Cn23,Hm,Cn85,Q10_law_Temperature_increase,Temperature,Temperature_T0,Trate_T0,ka_specificHm,kd_specificHm,ka_specific23,kd_specific23,ka_specific85,kd_specific85,kb23,kb23_85,kb85,gene_size_lin14,efficiency23_lin14,efficiency_85_lin14,efficiency23_85_lin14):
    ...
    Occupancy23*Occupancy85

    Occupancy_cooperativity = Kq*Kr*Kr*Cn23*Cn85/(Dn*Dn+Kr*Dn*(Cn23+Cn85)+Kq*Kr*Kr*Cn23*Cn85)
    
    Irate_NHR23_alone_site_1    = efficiency23_site1        *(1-np.exp(-kb23   *Occupancy23*OccupancyHm              /Imax))
    #print("Irate_NHR23_alone_site_1",Irate_NHR23_alone_site_1)
    Irate_NHR23_NHR85_site_1    = efficiency23_85_site1     *(1-np.exp(-kb23_85*Occupancy_cooperativity*OccupancyHm  /Imax))
...
def derive(Y,X):
    ...
    for i in range(len(Y)-1):
        ...
        a = dy/dx
        L_dY.append(a)
    L_dY.append(a) #this line duplicates the latest derivative value in order to have the same list's length for the Y and derivative of Y  
    return L_dY

def max_min_and_index(L):
    max_value = None
    min_value = None
    max_idx = None
    min_idx = None
...
def truncate_second_bump(Irate_one_LS,Lt):
    dIrate = derive(Irate_one_LS,Lt)
    max_value,min_value,max_idx,min_idx = max_min_and_index(Irate_one_LS)
    last_value = Irate_one_LS[len(Irate_one_LS)-1]
    tmin = Lt[min_idx]
    delta_T = Lt[max_idx]-tmin    
    for i in range(min_idx,len(Irate_one_LS)):
        ti = Lt[i]
        Irate_one_LS[i] = min_value + (last_value-min_value)*(ti-tmin)/delta_T
...
def timing_of_ONSET(Irate_one_LS,Lt):
    l = len(Irate_one_LS)
    max_value,min_value,max_idx,min_idx = max_min_and_index(Irate_one_LS)
    tON = Sigma_Computation(Lt,Irate_one_LS,1)
    for i in range(l):
        if(Irate_one_LS[i]<=
    return
...
plt.plot(Lt_whole_signal[0:indice_one_LS],L_NHR23[0:indice_one_LS] , color = 'r', label = 'NHR23 Pattern')
plt.plot(Lt_whole_signal[0:indice_one_LS],L_NHR85[0:indice_one_LS] , color = 'k', label = 'NHR85 Pattern')
plt.plot(Lt_whole_signal[0:indice_one_LS],L_LIN42[0:indice_one_LS] , color = [1,0,1], label = 'L_LIN42 Pattern')
...
bool_oneLS = 1 # boolean describing whether we plot 1 LS or 4 LS

plt.subplot(234)
#plt.title("Normalized lin4 miRNA concentration")
lin4_numeric = lin4_numeric[0:len(lin4_numeric)-1]
if(bool_oneLS==1):
    plt.plot(Lt_whole_signal[0:int(len(Lt_whole_signal)/4)],lin4_numeric[0:int(len(Lt_whole_signal)/4)] , color = 'b',label = 'num. sol.')#color_fig3_2, label = 'numerical solution') #/max(lin4_numeric)
    plt.plot(Lt_whole_signal[0:int(len(Lt_whole_signal)/4)],L_Irate_lin4[0:int(len(Lt_whole_signal)/4)], color = [0.3,0.3,0.3], label = 'Irate')
else:
    plt.plot(Lt_whole_signal,lin4_numeric , color = 'b',label = 'num. sol.')#color_fig3_2, label = 'numerical solution') #/max(lin4_numeric)
    plt.plot(Lt_whole_signal,L_Irate_lin4 , color = [0.3,0.3,0.3], label = 'Irate')
plt.xlabel('Scaled larval stages')
plt.ylabel('lin4(t)')
plt.legend()
plt.grid(True)
