...
def C_Processing(L_alpha,L_beta,L_gamma,C_t,dt):
...
while (cpt<N-1 or t<=t1-dt):
    ...
    C_t = C_Processing(L_alpha,L_beta,L_gamma,C_t,dt)
...
title = 'Activator = sine'
plt.title(title)
m = min(len(L_t),len(L_Activator))
Lt_norm = 4*np.array(L_t[0:m])/max(L_t[0:m])
plt.plot (Lt_norm , L_Activator[0:m] , c = 'r')# , label = 'Activator = sine')
plt.legend()
...
plt.plot(L_t[0:m], Pattern_function[0:m] , c = 'm')
plt.legend()
plt.ylim(-0.1,1.1)
plt.grid(True)
...
#title = "Activator Concentration in time ("+str(t1-t0)+"s)"
title = 'Activator = sine'
plt.title(title)
m = min(len(L_t),len(L_Activator))
Lt_norm = 4*np.array(L_t[0:m])/max(L_t[0:m])
