...
def Irate_NHR23_NHR85_lin4 (Imax,Dn,ka_specific_common_value,Kn,Kr,KCn23,Hm,Cn85,Q10_law_Temperature_increase,Temperature,Temperature_T0,Trate_T0,ka_specificHm,kd_specificHm,ka_specific23,kd_specific23,ka_specific85,kd_specific85,kb23,kb23_85,kb85,gene_size_site1,efficiency23_site1,efficiency_85_site1,efficiency23_85_site1,gene_size_site2,efficiency23_site2,efficiency23_85_site2,efficiency85_site2):
    ...
    Tdelay_in_response23_site2 = Time_delay_in_response(Trate,gene_size_site2) # Inherent delay in the response due to time it takes the RNA polymerase to transcribe the gene. The average delay is the product of the gene size and the RNA translocation rate.
...
def C_Processing(L_alpha,L_beta,L_gamma,L_Irate,L_Params_NHR85,C_t,t,dt):
    ...
    [lin4 , lin14 , LIN14 , NHR85 , LIN42]  = C_t
    ...
    if(NHR85_dt<0):
        NHR85_dt=0
    C_t_dt   = [lin4_dt , lin14_dt , LIN14_dt , NHR85_dt , LIN42]
...
NHR23_max = 505*2               # maximum value of the NHR23 signal
...
NHR85_max = 1122*2              # maximum value of the NHR85 signal
                                # This value comes from the experiments from Chris Hammel's lab about the NHRs/LIN42 to DNA binding.
...
LIN42_max = 916*2               # maximum value of the NHR23 signal
                                # This value comes from the experiments from Chris Hammel's lab about the NHRs/LIN42 to DNA binding.
...
kb23 = 1                                # UNKNOWN unit is "1/s" or "amount of transcription initiation rate at a given occupancy"
kb23_85 = 1                             # UNKNOWN unit is "1/s" or "amount of transcription initiation rate at a given occupancy"
kb85 = 1                                # UNKNOWN unit is "1/s" or "amount of transcription initiation rate at a given occupancy"
########################################################################################
...
gene_size_lin14 = 26406                     # This is the number of nucleotides of the gene coding sequence in the genome of the C.elegans.
...
gamma_NHR42_NHR85   = gamma_lin4_lin14
...
# This line adds the constant function that is there to see the post oscillations after all the 4 larval stages at the end of the Activator list
...
L_NHR23 = A23*(1+np.sin(2*np.pi*Lt_whole_signal+phi23))
L_NHR85 = A85*(1+np.sin(2*np.pi*Lt_whole_signal+phi85))
L_LIN42 = A42*(1+np.sin(2*np.pi*Lt_whole_signal+phi42))

C_t     = [n_lin4 , n_lin14 , n_LIN14 , L_NHR85[0] , L_LIN42[0]]
...
cpt_max = len(L_Activator_with_post_oscillation) # constant that ensures that all the simulation will be made on the whole activator duration.
integrity_boolean = 0 # This variable ensures that we won't print the warning message below in the while loop more than once.
while (cpt<cpt_max-1):
    Cn23 = L_NHR23[cpt] # The computations begin with the first value of the NHR23
    Cn85 = NHR85_numeric[cpt]
    LIN42 = L_LIN42[cpt] # The computations begin with the first value of the LIN42
    
    [Irate_lin4_site1,Irate_lin4_site2] = Irate_NHR23_NHR85_lin4(Imax,Dn,ka_specific_common_value,Kn,Kr,Cn23,Hm,Cn85,Q10_law_Temperature_increase,Temperature,Temperature_T0,Trate_T0,ka_specificHm,kd_specificHm,ka_specific23,kd_specific23,ka_specific85,kd_specific85,kb23,kb23_85,kb85,gene_size_site1,efficiency23_site1,efficiency85_site1,efficiency23_85_site1,gene_size_site2,efficiency23_site2,efficiency23_85_site2,efficiency85_site2)
    Irate_lin4 = Irate_lin4_site1 + Irate_lin4_site2
    ...
    [lin4_dt , lin14_dt , LIN14_dt , Cn85 , LIN42] = C_t # refreshing the current concentration values
...
plt.plot(Lt_whole_signal,np.array(lin4_numeric)  , color = color_fig3_2, label = 'numerical solution') #/max(lin4_numeric)
plt.xlabel('Scaled larval stages')
plt.ylabel('lin4(t)')
plt.legend()
plt.grid(True)
