...
D = 1e-10 #ENTER THE VALUE IN m²/s
...
Nx      = 200                    # number of space points between x0 and x1, the x boundaries on the x axis
...
cpt_max = 10                           # this corresponds to the number of curve computed to show only one curve among them (the last one from the 'cpt_max' curves will be shown)
dt      = (t1-t0)/(Nt-1)
t       = np.linspace(t0,t1,Nt,True)

#### Source and sink term profile ####
s0 = 0
s1 = 10000             # The source term has to have the same time unit as D (the diffusion constant)
source_term = constant(s0,Nx)

Ncell = 10
i_step = int((Nx-1)/(Ncell+1))
print(i_step)
...
#plt.subplot(331)
ax = fig.add_subplot(221) # Add a subplot
plt.subplot(221)
...
#plt.subplot(334)
ax = fig.add_subplot(222) # Add a subplot
plt.subplot(222)
