...
sys_epsilon_value = sys.float_info.min
print("System epsilon value = ",sys_epsilon_value)
...
#bool_stability_condition_correction = int(input("Automaticaly correct time step length to keep stability condition valid ? (y/n)"))
bool_stability_condition_correction = 1

def main(TIME_STEP_LENGTH):
    ...
    if (bool_stability_condition_correction == 0):
        raise RuntimeError("Stability is not guarenteed.")
    elif (bool_stability_condition_correction == 1):
        TIME_STEP_LENGTH = STABILITY_SAFETY_FACTOR * maximum_possible_time_step_length - sys_epsilon_value
    else :
        raise RuntimeError("Stability is not guarenteed.")
